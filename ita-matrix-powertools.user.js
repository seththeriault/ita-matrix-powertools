...
function createUsersettings() {
  ...
  settingscontainer.innerHTML = '<div style="display:inline-block;float:left;cursor:pointer;" id="passengerVisToggler">Passengers (<label id="mtpPaxCount">1a</label>)</div><div id="mptStartparse" class="invis" style="margin-left:20px;display:none;cursor:pointer">Editor-Mode:Parse!</div><div id="mtpNotification" style="margin-left:50px;display:inline-block;"></div><div style="display:inline-block;float:right;"><div id="settingsVisToggler" style="display:inline-block;cursor:pointer;">Settings</div> (v'+mptSettings.version+') <div id="mptCabintoggler" style="display:inline-block;">(Cabin: <label id="mptCabinMode" style="width:30px;text-align:center;cursor:pointer;display:inline-block">Auto</label>)</div></div><div id="mptSettings" class="invis" style="display:none;border-top: 1px dotted grey;"></div><div id="mptPassengers" class="invis" style="display:none;border-top: 1px dotted grey;"></div>';
  ...
  document.getElementById('passengerVisToggler').onclick=function(){toggleVis(document.getElementById("mptPassengers"));};
  // Build settings
  target=document.getElementById("mptSettings");
  str ='<div id="mptrestoredefault" style="text-align:right;font-weight:bold;text-decoration:underline;">Restore Defaults</div>';
  ...
  str +='<div id="mpttimeformat">Time Format: <label style="cursor:pointer;">'+printSettingsvalue("timeformat")+'</label></div>';
  ...
  str +='<div id="mptenableInlineMode">Inline Mode: <label style="cursor:pointer;">'+printSettingsvalue("enableInlineMode")+'</label></div>';
  str +='</div><div style="float:left;width:25%">';
  ...
  str +='</div><div style="clear:both"></div></div>';
  str +='<div style="text-align:center;font-weight:bold">**** Airline Locale / Edition: ****</div>';
  str +='<div style="margin:5px 0">';
  str +='<div id="mptaaEdition" style="width:33%;float:left;">American (Europe/Asia/Pacific): <label style="cursor:pointer;">'+printSettingsvalue("aaEdition")+'</label></div>';
  str +='<div id="mptaac1Edition" style="width:33%;float:left;">American (America/UK): <label style="cursor:pointer;">'+printSettingsvalue("aac1Edition")+'</label></div>';
  str +='<div id="mptacEdition" style="width:33%;float:left;">Air Canada: <label style="cursor:pointer;">'+printSettingsvalue("acEdition")+'</label></div>';
  str +='<div id="mptafEdition" style="width:33%;float:left;">Air France: <label style="cursor:pointer;">'+printSettingsvalue("afEdition")+'</label></div>';
  str +='<div id="mptazEdition" style="width:33%;float:left;">Alitalia: <label style="cursor:pointer;">'+printSettingsvalue("azEdition")+'</label></div>';
  ...
  str +='<div id="mptbaEdition" style="width:33%;float:left;">British Airways (Locale): <label style="cursor:pointer;">'+printSettingsvalue("baEdition")+'</label></div>';
  str +='<div id="mptczEdition" style="width:33%;float:left;">China Southern: <label style="cursor:pointer;">'+printSettingsvalue("czEdition")+'</label></div>';
  str +='<div id="mptdlEdition" style="width:33%;float:left;">Delta: <label style="cursor:pointer;">'+printSettingsvalue("dlEdition")+'</label></div>';
  str +='<div id="mptibEdition" style="width:33%;float:left;">Iberia: <label style="cursor:pointer;">'+printSettingsvalue("ibEdition")+'</label></div>';
  str +='<div id="mptklEdition" style="width:33%;float:left;">KLM: <label style="cursor:pointer;">'+printSettingsvalue("klEdition")+'</label></div>';
  str +='<div id="mptlaEdition" style="width:33%;float:left;">LATAM: <label style="cursor:pointer;">'+printSettingsvalue("laEdition")+'</label></div>';
  str +='<div id="mptlhEdition" style="width:33%;float:left;">Lufthansa: <label style="cursor:pointer;">'+printSettingsvalue("lhEdition")+'</label></div>';
  str +='<div id="mptlxEdition" style="width:33%;float:left;">Swiss: <label style="cursor:pointer;">'+printSettingsvalue("lxEdition")+'</label></div>';
  str +='<div id="mptqfEdition" style="width:33%;float:left;">Qantas Airways: <label style="cursor:pointer;">'+printSettingsvalue("qfEdition")+'</label></div>';
  str +='<div style="clear:both"></div></div>';
  str +='<div style="text-align:center;font-weight:bold">**** Airline Currency: ****</div>';
  str +='<div style="margin:5px 0">';
  str +='<div id="mptaac1Currency" style="width:33%;float:left;">American (America/UK): <label style="cursor:pointer;">'+printSettingsvalue("aac1Currency")+'</label></div>';
  ...
  str +='<div id="mptqfCurrency" style="width:33%;float:left;">Qantas Airways: <label style="cursor:pointer;">'+printSettingsvalue("qfCurrency")+'</label></div>';
  str +='<div style="clear:both"></div></div>';
  str +='<div style="text-align:center;font-weight:bold"><label id="configcloser" style="cursor:pointer;text-decoration:underline;">Close</label><div>';
  target.innerHTML=str;

    // these onClick event handlers need only be added once:
    document.getElementById('mptrestoredefault').onclick=function(){restoreDefaultSettings();};
...
(async ()=>{
  if (typeof GM.info === "undefined") {
    ...
    if (mptSavedUserSettings) {
      ...
      mptUserSettings.laCurrency = (mptSavedUserSettings.laCurrency === undefined ? mptUserSettings.laCurrency : mptSavedUserSettings.laCurrency);
...
function printLA() {
  ...
  var createUrl = function (edition) {
    ...
    for (var i=0;i<currentItin.itin.length;i++) {
      ...
      laUrl += '{"amount":"'+currentItin.price+'","currency":"'+mptUserSettings.laCurrency+'","segments":[';
      var mincabin=3;
      // walks each leg
      for (var j=0;j<currentItin.itin[i].seg.length;j++) {
        //walks each segment of leg
        ...
        laUrl +='{"departure_airport":"'+currentItin.itin[i].seg[j].orig
              +'","flight_number":"'+currentItin.itin[i].seg[j].fnr
              +'","departure_date":"'+ currentItin.itin[i].seg[j].dep.year.toString()+'-'+ ('0'+currentItin.itin[i].seg[j].dep.month.toString()).slice(-2) +'-'+ ('0'+currentItin.itin[i].seg[j].dep.day.toString()).slice(-2)
              +'","arrival_airport":"'+currentItin.itin[i].seg[j+k].dest
              +'","farebasis":"'+currentItin.itin[i].seg[j].farebase
              +'","marketing_airline":"'+currentItin.itin[i].seg[j].carrier
              +'","class":"'+currentItin.itin[i].seg[j].bookingclass
              +'","arrival_date":"'+ currentItin.itin[i].seg[j].arr.year.toString()+'-'+ ('0'+currentItin.itin[i].seg[j].arr.month.toString()).slice(-2) +'-'+ ('0'+currentItin.itin[i].seg[j].arr.day.toString()).slice(-2)
              +'"},';
    ...
    var laPassengers = '"passengers":{"numberAdults":"'+pax.adults+'","numberInfants":"'+pax.infLap+'","numberChildren":"'+pax.children.length+'"},';
    // Compile the final URL (and encode it):
    laUrl = 'https://ssl.lan.com/cgi-bin/compra/paso4.cgi?forced_home='+edition+'&sessionParameters=%7B' + encodeURIComponent(laPassengers) + encodeURIComponent(laUrl.substring(0,laUrl.length-1)) +']}}&utm_medium=metasearch&utm_source=gfs&utm_campaign=US_deeplink_s4&gclsrc=gf';
    return laUrl;
  };
  ...
  var extra = ' <span class="pt-hover-container">[+]<span class="pt-hover-menu">';
  extra += laEditions.map(function (obj, i) { return '<a href="' + createUrl(obj.value) + '" target="_blank">' + obj.name +'</a>'; }).join('<br/>');
  extra += '</span></span>';
  if (mptUserSettings.enableInlineMode==1){
...
function printQF() {
  /* Qantas partner deep-link */
   ...
   var createUrl = function (edition,currency) {
     ...
     var urlBase = "https://book.qantas.com/qf-booking/dyn/air/tripflow.redirect?APPLICATION_NAME=SDC";
     // Add edition / locale:
     urlBase += "&USER_LANG=EN&USER_LOCALE="+edition;
     // Add class(es) of service:
     urlBase += "&travelClass=" + travelClass[(mptSettings.cabin==="Auto" ? mincabin : getForcedCabin())];
     // Add passenger info:
     urlBase += "&numberOfAdults="+pax.adults+"&numberOfChildren="+nbrChildren.toString()+"&numberOfInfants="+pax.infLap
     // Add currency:
...
function printCheapOair(){
  ...
  for (var i=0; i < currentItin.itin.length; i++) {
    ...
    for (var j=0; j < currentItin.itin[i].seg.length; j++) {
      ...
      coaUrl += '&cbn' +seg+'='+cabins[(mptSettings.cabin==="Auto" ? cabins[currentItin.itin[i].seg[j].cabin] : getForcedCabin())];
...
function printExpedia(){
  var pax=validatePaxcount({maxPaxcount:9, countInf:true, childAsAdult:18, sepInfSeat:false, childMinAge:2});
    ...
    let editions = [{name:"expedia.com",host:"expedia.com"},{name:"orbitz.com",host:"orbitz.com"},{name:"expedia.ca",host:"expedia.ca"},{name:"expedia.de",host:"expedia.de"},{name:"expedia.it",host:"expedia.it"},{name:"expedia.es",host:"expedia.es"},{name:"expedia.co.uk",host:"expedia.co.uk"},{name:"expedia.dk",host:"expedia.dk"},{name:"expedia.mx",host:"expedia.mx"},{name:"expedia.fi",host:"expedia.fi"},{name:"expedia.fr",host:"expedia.fr"}, {name:"expedia.no",host:"expedia.no"},{name:"expedia.nl",host:"expedia.nl"},{name:"expedia.ch",host: "expedia.ch"},{name:"expedia.se",host:"expedia.se" },{name:"expedia.at",host:"expedia.at"},{name:"expedia.co.jp",host:"expedia.co.jp"}];
    let expediaClasses = ['coach', 'premium', 'business', 'first'];
    let minCabin = 3;
    let ExpediaCreateUrl = function(expediaBase) {
      let segUrl = "";
      for (var i=0; i < currentItin.itin.length; i++) {
        segUrl += "&legs%5B" + i + "%5D.departureAirport=" + currentItin.itin[i].orig;
        segUrl += "&legs%5B" + i + "%5D.arrivalAirport=" + currentItin.itin[i].dest;
        segUrl += "&legs%5B" + i + "%5D.departureDate=" + currentItin.itin[i].arr.year.toString() + '-' + ("0"+currentItin.itin[i].dep.month).slice(-2) + '-' + ("0"+currentItin.itin[i].dep.day).slice(-2);
        for (var j=0; j < currentItin.itin[i].seg.length; j++) {
          segUrl += ("&legs%5B" + i + "%5D.segments%5B" + j + "%5D=" + currentItin.itin[i].seg[j].dep.year.toString()+ '-' + ("0"+currentItin.itin[i].seg[j].dep.month).slice(-2)+ '-' +("0"+currentItin.itin[i].seg[j].dep.day).slice(-2)
              + '-' + expediaClasses[(mptSettings.cabin==="Auto" ? minCabin : getForcedCabin())] + '-'
      ...
      // Build the URL:
      let baseUrl = "https://www." + expediaBase + "/Flight-Search-Details?action=dl&trip=MultipleDestination";
      // Add travel class to URL:
      baseUrl += "&cabinClass=" + expediaClasses[(mptSettings.cabin==="Auto" ? minCabin : getForcedCabin())];
...
function printMomondo() {
    ...
    var MomondoCreateUrl = function(host) {
        ...
        // Add travel class to URL:
        MomondoUrl += momondoTravelClass[(mptSettings.cabin==="Auto" ? mincabin : getForcedCabin())] + "/";
        // Add passenger info to URL:
...
function printSkyscanner() {
    ...
    var SkyscannerCreateUrl = function(market) {
      ...
      for (var i = 0; i < currentItin.itin.length; i++) {
        ...
        // Add the segments:
        skyscannerUrl += '/' + currentItin.itin[i].dep.year
      ...
      // Add passenger info:
      skyscannerUrl += '?adults=' + currentItin.numPax + 'adultsv2=' + currentItin.numPax;
      // Add cabin / class of service:
      skyscannerUrl += "&cabinclass=" + skyscannerTravelClass[(mptSettings.cabin==="Auto" ? mincabin : getForcedCabin())];
      // Add locale ("market"):
