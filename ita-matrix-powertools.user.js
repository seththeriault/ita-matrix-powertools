...
function printExpedia(){
  var pax=validatePaxcount({maxPaxcount:9, countInf:true, childAsAdult:18, sepInfSeat:false, childMinAge:2});
    ...
    let ExpediaCreateUrl = function(baseUrl) {
      let url = "https://www." + baseUrl + "/Flight-Search-Details?action=dl&trip=MultipleDestination&cabinClass=coach&adults=" + pax.adults;
      for (var i=0; i < currentItin["itin"].length; i++) {
        ...
        for (var j=0; j < currentItin["itin"][i]["seg"].length; j++) {
          url += ("&legs%5B" + i + "%5D.segments%5B" + j + "%5D=" + currentItin["itin"][i]["seg"][j]["dep"]["year"].toString()+ '-' + ("0"+currentItin["itin"][i]["seg"][j]["dep"]["month"]).slice(-2)+ '-' +("0"+currentItin["itin"][i]["seg"][j]["dep"]["day"]).slice(-2) + '-coach-' + currentItin["itin"][i]["seg"][j]["orig"] + '-' + currentItin["itin"][i]["seg"][j]["dest"] + '-' + currentItin["itin"][i]["seg"][j]["carrier"] + '-' + currentItin["itin"][i]["seg"][j]["fnr"]).toLowerCase()
        }
      }
     return url
    }
